import "server-only";
import type { Bead, CreateInput, UpdateInput, DepType, Dependency, Comment } from "./schema";
import type { BeadsStore, DoctorInfo } from "./store";

/**
 * In-memory store seeded from the Claude Design export's prototype dataset.
 * Used when bd isn't installed or when demo mode is forced. Mutations persist
...
function seed(): Bead[] {
  return [
    ...
    D("bd-1a09.3", "README + setup docs", "story", "deferred", 4, "stevey", "", "bd-1a09", { description: "Setup, bd version range, troubleshooting." }),
    // loose beads
